...
export const Account: React.FC = () => {
  ...
  const handleSaveProfile = (e: React.FormEvent) => {
    ...
    if (editForm.newPassword) {
      ...
      profileData.confirmNewPassword = editForm.confirmNewPassword;
      ...
      key === 'delivered'
        ...
        : key === 'on_the_way'
          ...
          : key === 'cancelled'
            ...
            : 'var(--primary)';
    return (
      <span className="badge badge-pending" style={{ borderColor: color, background: color }}>
        <span style={{ color: '#fff', textShadow: '0 0 2px rgba(0,0,0,0.5)', boxShadow: `0 0 10px ${color}, 0 0 20px ${color}` }}>
  ...
  const getPaymentBadge = (order: Order) => {
    ...
    return (
      <span className="badge badge-paid" style={{ borderColor: color, background: color }}>
        <span style={{ color: '#fff', textShadow: '0 0 2px rgba(0,0,0,0.5)', boxShadow: `0 0 10px ${color}, 0 0 20px ${color}` }}>
            ...
            <div className="glass" style={{ padding: '2rem', border: '1px solid var(--border)' }}>
              ...
              {isEditing ? (
                <form onSubmit={handleSaveProfile} style={{ display: 'flex', flexDirection: 'column', gap: '1rem', fontSize: '0.9rem' }}>
                  ...
                  {editForm.newPassword && (
                    <div>
                      <label style={{ color: passwordsMatch ? 'var(--text-dark)' : 'var(--error)', display: 'block', fontSize: '0.75rem', textTransform: 'uppercase', fontWeight: 600, marginBottom: '0.3rem' }}>
                        Confirmar Nova Senha (obrigatório para alterar senha)
                      </label>
                      <input
                        type="password"
